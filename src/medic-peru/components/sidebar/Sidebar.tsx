        ...
        <aside className="h-screen mr-5">
            <nav className="h-full flex flex-col bg-white border-r shadow-sm">
                <div className="p-4 pb-2 flex justify-between items-center mt-3">
                    ...
                    >
                        {expanded ? <ChevronFirst className="text-black" /> : <ChevronLast className="mr-1.5 text-black" />}
                    </button>
                </div>
    ...
    return (
      ...
      `}
        >
            {icon}
            <span
                className={`overflow-hidden text-lg transition-all ${expanded ? "w-52 ml-3" : "w-0"
                    }`}
            >
                {text}
            </span>
